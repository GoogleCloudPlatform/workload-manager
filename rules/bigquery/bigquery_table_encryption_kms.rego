...
#######################################################################
# DETAILS: Ensure table is using KMS to control data encryption.
# SEVERITY: Medium
# ASSET_TYPE: bigquery.googleapis.com/Table
# TAGS: BigQuery, Manageability, Operations, Security
########################################################################

package google.bigquery.table.enforce.encryption
...
deny [{"msg": message, "details": metadata}] {
	...
	key_name := lib.get_default(asset.resource.data.encryptionConfiguration, "kmsKeyName", "NOTFOUND")
